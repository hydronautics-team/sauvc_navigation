...
MAX_LINEAR_SPEED_L = 30
MAX_LINEAR_SPEED_A = 25
MAX_ANGULAR_SPEED_L = 35
MAX_ANGULAR_SPEED_A = 35
...
def normPolarAngle(angle) :
    pass

def calcMove(circles, startPosition, selfAngleAll, endPoint, needToDraw, targetDeep, speedLongRatio=1, degCost=DEG_COST) :
    selfAngle = selfAngleAll[2]
    startPoint = startPosition
    astar = AStar.AStar()
    path, image = astar.main(startPoint, endPoint, circles, needToDraw, False, selfAngle, degCost)
    draw = None
    if needToDraw : draw = ImageDraw.Draw(image)

    selfAngle %= 360

    #print('selfAngle:', selfAngle)
    ...
    if isinstance(path[0], AStar.Line):
        ...
        length = distance(path[0].x1, path[0].y1, path[0].x2, path[0].y2) * speedLongRatio
    ...
    if isinstance(path[0], AStar.Arcc):
        ...
        polarLenght = min(abs(path[0].p1-path[0].p2), abs(360-path[0].p1-path[0].p2)) * speedLongRatio
...
def normVectorFromArcc(vector, needToStop, selfAngleAll, speedFinal) :
    ...
    speed[2] = min(speed[2], MAX_VERTICAL_SPEED)
    speedFinal[2] = min(speedFinal[2], MAX_VERTICAL_SPEED)

    # длинна пути
    pathLenth = speedAbs = (speed[0] ** 2 + speed[1] ** 2) ** 0.5

    # замедляемся, если нужно
    needToStop = False  # Не нужно
    if needToStop:
    ...
    if speedAbs > maxLinearSpeed or (speedAbs != 0 and needToStop == False):
        ...
        speedFinal[0] = speedFinal[0] / speedAbs * maxLinearSpeed
        speedFinal[1] = speedFinal[1] / speedAbs * maxLinearSpeed

    # запрет движения при расхождении угла
    if abs(angularSpeed[2]) > maxMoveAngle:
        speed = [0, 0, speed[2]]
        speedFinal = [0, 0, speedFinal[2]]

    # изменение модуля угловой скорости
    if abs(angularSpeed[2]) > maxAngularSpeed:
...
def normVectorFromLine(vector, needToStop, selfAngleAll, speedFinal) :
    ...
    if abs(angularSpeed[2]) > maxAngularSpeed:
        angularSpeedAbs = abs(angularSpeed[2])
        #print('angularSpeedAbs:', angularSpeedAbs)
        #print('del:', angularSpeedAbs * maxAngularSpeed)
        angularSpeed[2] = angularSpeed[2] / angularSpeedAbs * maxAngularSpeed
