...
class AStar:
    ...
    @staticmethod
    def checkLine2(circles, x1, y1, x2, y2, circleIndexes, draw):
        # x1, y1, x2, y2 = x2, y2, x1, y1
        ...
        for i in range(len(circles)):
            ...
            if (v1x ** 2 + v1y ** 2) == 0:
                # AStar.drawCircle(draw, x1, y1, 2, 'orange')
                if ((x1 - circle[0]) ** 2 + (y1 - circle[1]) ** 2) ** 0.5 < circle[2] - COLLISION_EPSILON:
                    return i
                # else : return True
                else:
                    continue
            ...
            if ((circle[0] - x) ** 2 + (circle[1] - y) ** 2) ** 0.5 < circle[2] - COLLISION_EPSILON: return i
    ...
    @staticmethod
    def start_path(circles, open_set, current_node, end, draw, dr, isStartPoint, mainPolarAngle, degCost):
        #print('\nstart_path')
        ...
        for neighbor in neighborsLine :
            ...
            if isStartPoint :
                polarAngleBetween = AStar.getPolarAngle(current_node.x, current_node.y, neighbor.x, neighbor.y)
                degDistance = min(abs(mainPolarAngle - polarAngleBetween), 360 - abs(mainPolarAngle - polarAngleBetween))
                temp_g += degDistance * degCost
        ...
        for neighbor in neighborsCircle:
            ...
            hordD = AStar.hordD(circles, current_node, neighbor)[0]
            l = 2 * math.pi * circles[neighbor.circleIndex].r
            lenth = hordD / l * 360
            #hordD += lenth * degCost
            temp_g = current_node.g + hordD
    ...
    def main(self, start, end, circles_, needToDraw, needToDrawAllLines, mainPolarAngle, degCost):
        ...
        # if degCost != 0 :
        #     r = 50
        #     x2 = startNode.x + math.cos(mainPolarAngle/360*2*math.pi) * r
        #     y2 = startNode.y + math.sin(mainPolarAngle / 360 * 2 * math.pi) * r
        #     AStar.drawLine(draw, startNode.x, startNode.y, x2, y2, 'green', 3)

        #im.show()
        #if (maxDeepIndex < len(circles)) :
            #print(maxDeepIndex, len(circles))
        open_set, current_node, final_path = AStar.start_path(self.circles, open_set, current_node, endNode,
                                                              draw, max_deep,True, mainPolarAngle, degCost)
        while len(open_set) > 0:
            open_set, current_node, final_path = AStar.start_path(self.circles, open_set, current_node,
                                                                  endNode, draw, max_deep, False,
                                                                  mainPolarAngle, degCost)
...
if __name__ == "__main__":
    ...
    time1 = time.time()
    final_path, im = a_star.main(startPoint, endPoint, circles, False, False, angle, degCost)
    print("Time to make a path and draw:", time.time() - time1)

    time1 = time.time()
    final_path, im = a_star.main(startPoint, endPoint, circles, False, True, angle, degCost)
    print("Time to make a path and draw:", time.time() - time1)

    time1 = time.time()
    final_path, im = a_star.main(startPoint, endPoint, circles, True, False, angle, degCost)
    print("Time to make a path and draw:", time.time()-time1)
    im.show()
    im.save('draw-arc.png')

    time1 = time.time()
    final_path, im = a_star.main(startPoint, endPoint, circles, True, True, angle, degCost)
    print("Time to make a path and draw:", time.time() - time1)
    im.show()
